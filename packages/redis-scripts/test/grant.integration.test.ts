...
test("grant.lua against live Redis", { skip: !REDIS_URL }, async (t) => {
  const {
    ...
    parsePeekResult, parsePlaceResult, canvasKeys, gaugeKey, DEFAULT_GAUGE, grantCharges,
  ...
  // FEN-1616: the gauge is per-(canvas, user), so grant and place must address the
  // SAME canvas-scoped key or the grant would land on a different bucket.
  const gk = gaugeKey(canvasId, userId);
  // Effective max the gateway passes (base + bonus). Start at base 5.
  const P = { ...DEFAULT_GAUGE, refillIntervalMs: 1000, gaugeMax: 5 };
  const keysToClear = [K.pixels, K.meta, K.stream, gk];
  ...
  const grant = async (n: number, nowMs: number, gauge = P) => {
    const { keys, argv } = grantArgs({ nowMs, gauge, userId, grant: n, canvasId });
  ...
  assert.equal(Number(await redis.hget(gk, "c")), 5);
  ...
  assert.equal(Number(await redis.hget(gk, "c")), 6);
});
