...
test("moderateArgs lays out KEYS + ARGV in the order moderate.lua expects", () => {
  ...
  const { keys, argv } = moderateArgs({
    ...
    canvasId: CID,
    cells,
    deltaChannel: canvasDeltaChannel(CID),
    actorUserId: "mod-1",
  ...
  assert.deepEqual(argv, [
    "512", "512", "32", canvasDeltaChannel(CID), "mod-1", "1700000000000", "2",
    "1", "2", "0",
    "3", "4", "7",
  ]);
});
...
test("moderateArgs defaults actor/channel and can skip the durable stream", () => {
  ...
  assert.equal(argv[3], canvasDeltaChannel(CID)); // default channel derived from canvasId
...
test("placeArgs lays out the per-canvas KEYS and threads userId into ARGV (FEN-54)", () => {
  ...
  // KEYS = [pixels, gauge, meta, frozen, stream, bans, op] — the gauge is now
  // per-(canvas, user) (FEN-1616); the bans set (CA6) is always present; the op
  // slot is "" with no opId (CA5: idempotency off).
  assert.deepEqual(keys, [K.pixels, gaugeKey(CID, "u1"), K.meta, K.frozen, K.stream, K.bans, ""]);
  assert.equal(keys.length, 7);
  // userId is ARGV[13] (argv[12]), carried onto the stream record; opId/opTtl
  // (ARGV[14]/[15]) follow it.
  assert.equal(argv[12], "u1");
  assert.equal(argv[11], canvasDeltaChannel(CID)); // ARGV[12] = deltaChannel, derived from canvasId
});
