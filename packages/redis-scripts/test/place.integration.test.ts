...
test("place.lua + refill-peek.lua against live Redis", { skip: !REDIS_URL }, async (t) => {
  ...
  const keysToClear = [K.pixels, K.meta, K.stream, gaugeKey(canvasId, userId)];
  ...
  const peek = async (nowMs: number) => {
    const { keys, argv } = peekArgs({ nowMs, gauge: P, userId, canvasId });
  ...
  keysToClear.push(gaugeKey(canvasId, upgraded));
...
test("place.lua F4: ban enforcement (CA6) + idempotency (CA5) against live Redis", { skip: !REDIS_URL }, async (t) => {
  const { PLACE_LUA, placeArgs, parsePlaceResult, canvasKeys, gaugeKey, DEFAULT_GAUGE } =
    await import("../src/index.ts");
  const { default: Redis } = await import("ioredis");
  ...
  const keysToClear = [
    ...
    gaugeKey(canvasId, banned), gaugeKey(canvasId, op),
  ...
  assert.equal(Number(await redis.exists(gaugeKey(canvasId, banned))), 0, "banned placement touches no gauge");
  ...
  assert.equal(Number(await redis.hget(gaugeKey(canvasId, op), "c")), 4, "replay does NOT consume a second charge");
