 ...
 * Each gateway instance SETs `presence:inst:{id}` to its local viewer count
 * with a TTL; crashed instances self-heal when the TTL expires. Returns 0 when
 * no instance is live. Single implementation shared by the gateway (which also
 * writes presence) and the worker (which only reads for viewerCount flush).
 */
