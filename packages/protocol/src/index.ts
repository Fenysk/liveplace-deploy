 ...
 *
 * v3 (D8, FEN-2050): removed the deprecated transitional `seq` integer op-id
 * from `place`, `ack`, and `error` frames (superseded by `cid` per FEN-63).
 * The binary-frame `seq` (Redis write counter) and the `welcome`/`resync` JSON
 * `seq` (reconnect cursor) are unaffected — they are the same thing.
 */
...
// ─────────────────────────────────────────────────────────────────────────────
// Canvas selection (S0 — FEN-1560/1562)
//
// The client transmits the canvasId as a query param on the WS upgrade:
//   wss://host/ws?token=<jwt>&canvas=<canvasId>
// Absent → DEFAULT_CANVAS_ID (retro-compat local/single-canvas).
// ─────────────────────────────────────────────────────────────────────────────

/** Query-param name carrying the canvasId on the WS upgrade URL. */
export const CANVAS_QUERY_PARAM = "canvas";

/**
 * Allowlist for canvasId values. Prevents Redis key injection — the canvasId
 * is used as a segment in keys like `canvas:{canvasId}:pixels`.
 * Only alphanumeric, underscore, and hyphen; 1–64 chars.
 */
export function isValidCanvasId(s: string): boolean {
...
export type ClientMessage =
   ...
   */
  | { t: "place"; x: number; y: number; color: number; cid?: string }
  | { t: "ping" }
  /**
...
export type ServerMessage =
   ...
   * pixel — it is the ratified correlation.
   */
  | ({ t: "ack"; cid?: string } & GaugeState)
  ...
  /**
   * The canvas geometry changed (resize). Sent to all currently-connected
   * clients on that canvas so they can re-render at the new size immediately,
   * without waiting for a reconnect / new `welcome`. Server → client only.
   *
   * ADDITIVE & non-breaking: a client that does not know `t === "dimsChanged"`
   * ignores it — does not bump PROTOCOL_VERSION (same rule as `moderationEvent`).
   */
  | { t: "dimsChanged"; width: number; height: number }
  /**
   ...
   * tagged; omitted for errors not tied to a `place` (e.g. malformed frame).
   */
  | { t: "error"; code: ErrorCode; message: string; cid?: string }
  | { t: "pong" };
