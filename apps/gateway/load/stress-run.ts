 ...
 *     pnpm --filter @canvas/gateway exec tsx load/stress-run.ts
 *
 * ⚠️ TARGET = a DEDICATED STRESS GATEWAY, not the primary host. The prod gateway is
 * SINGLE-CANVAS (serves its `GATEWAY_CANVAS_ID`, ignores the WS path slug), so you
 * CANNOT isolate writes by changing the URL — every place against the primary
 * gateway lands on the live `default` art. Run a second gateway instance (same image,
 * same prod Redis + Convex) with `GATEWAY_CANVAS_ID=stress-YYYYMMDD` on its own WS
 * endpoint and point WS_URL there. Writes then live in `canvas:stress-YYYYMMDD:*`
 * (Redis) + Convex placements authored by `loadtest:user:*` → isolated + cleanable;
 * the live canvas is untouched. (Pattern: docker-compose.loadtest.fanout.yml.)
