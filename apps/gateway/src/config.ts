...
export interface GatewayConfig {
   ...
   * Redis keys (`canvasKeys`, ADR-0003). Since FEN-1564/1613 this is the canvas
   * Convex `_id` (NOT the human `slug`; ADR-0001's "canvasId == slug" is
   * historical). From GATEWAY_CANVAS_ID; falls back to DEFAULT_CANVAS_ID for
   * local single-canvas smoke. In multi-canvas serving the per-connection
   * canvas is resolved from the WS `?canvas=<_id>` param, not this default.
   */
  canvasId?: string;
  ...
  /**
   * Approximate MAXLEN backstop on the durable placement stream (FEN-651/A8). The
   * steady-state trim is exact (worker XTRIM MINID after each flush); this cap only
   * bounds Redis memory when the worker is down. See docs/contracts/retention.md.
   */
  streamMaxLen: number;
...
export function loadConfig(): GatewayConfig {
  ...
  return {
    ...
    streamMaxLen: num("STREAM_MAXLEN", 1_000_000),
    presenceRefreshMs,
    presenceTtlMs,
