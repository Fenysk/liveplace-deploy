...
import { DEFAULT_GAUGE, gaugeKey, type GaugeParams } from "@canvas/redis-scripts";
...
test("ConvexGaugeBonusSource queries points.getGaugeBonus with the PER-CALL canvasId (FEN-1616)", async () => {
  ...
  // The constructor canvasId is only a fallback; the per-connection canvasId
  // passed at call time is authoritative so a user on canvas B never reads the
  // bonus resolved for canvas A.
  const source = new ConvexGaugeBonusSource(client, "gateway-default");
  assert.equal(await source.getGaugeBonus("user-A", "canvas-B"), 4);
  assert.deepEqual(calls, [{ name: "points:getGaugeBonus", args: { canvasId: "canvas-B", userId: "user-A" } }]);
});

test("ConvexGaugeBonusSource falls back to the constructor canvasId when none is passed", async () => {
  const calls: Array<{ name: string; args: Record<string, unknown> }> = [];
  const client: ConvexQueryClient = {
    async query(name, args) {
      calls.push({ name, args });
      return { gaugeMaxBonus: 2 };
    },
  };
  const source = new ConvexGaugeBonusSource(client, "gateway-default");
  assert.equal(await source.getGaugeBonus("user-A"), 2);
  assert.deepEqual(calls, [
    { name: "points:getGaugeBonus", args: { canvasId: "gateway-default", userId: "user-A" } },
  ]);
});
...
test("SessionGauge starts at base, then applies the bonus on refresh (CA2)", async () => {
  ...
  const g = new SessionGauge(source, "user-A", 20, "canvas-1");
...
test("SessionGauge for an anonymous viewer (userId null) never queries the source (FEN-53)", async () => {
  ...
  const g = new SessionGauge(source, null, 20, "canvas-1");
  assert.equal(g.effectiveGaugeMax, 20);
...
test("grant runner forwards (gaugeKey, now, interval, amount, max, grant, ttl) and parses the snapshot", async () => {
  ...
  const out = await runner.grant("user-Z", "canvas-1", GP, 1, 1_700_000_000_000);
  ...
  // KEYS[1] = the per-(canvas, user) gauge hash; ARGV carries the effective max + grant.
  assert.equal(call.args[0], gaugeKey("canvas-1", "user-Z"));
...
test("grant runner reloads the script once on NOSCRIPT and retries", async () => {
  ...
  const runner = new IoredisGaugeGrantRunner(fake as never);
  const out = await runner.grant("user-Z", "canvas-1", GP, 2, 1_700_000_000_000);
...
test("peek runner forwards (gaugeKey, now, interval, amount, max) and parses the snapshot", async () => {
  ...
  const out = await runner.peek("user-Z", "canvas-1", GP, 1_700_000_000_000);
  assert.deepEqual(out, { charges: 21, max: 21, cooldownUntil: 0 });
  assert.equal(fake.loads, 1); // SCRIPT LOAD once
  assert.equal(fake.calls.length, 1);
  const [call] = fake.calls;
  assert.ok(call);
  assert.equal(call.numKeys, 1);
  // KEYS[1] = the per-(canvas, user) gauge hash; ARGV = [now, interval, amount, effMax].
