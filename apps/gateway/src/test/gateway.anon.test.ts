...
test("malformed JSON ⇒ bad_request error (not internal — ADR-0006, PROTOCOL_VERSION 2)", async () => {
  const gw = new Gateway(cfg(), undefined, createFakeRedis().pair);
  await gw.start();
  try {
    const token = await mint("user-m");
    const { ws, texts } = await connect(gw.boundPort, { token });
    await waitFor(ws, texts, (m) => m.t === "welcome");
    ws.send("this is not json {{{");
    const err = await waitFor(ws, texts, (m) => m.t === "error");
    assert.ok(err.t === "error");
...
test("unknown message type ⇒ bad_request error (not internal — ADR-0006, PROTOCOL_VERSION 2)", async () => {
  const gw = new Gateway(cfg(), undefined, createFakeRedis().pair);
  await gw.start();
  try {
    const token = await mint("user-u");
    const { ws, texts } = await connect(gw.boundPort, { token });
    await waitFor(ws, texts, (m) => m.t === "welcome");
    ws.send(JSON.stringify({ t: "not_a_real_message_type" }));
    const err = await waitFor(ws, texts, (m) => m.t === "error");
    assert.ok(err.t === "error");
    assert.equal(err.code, "bad_request");
    ws.close();
