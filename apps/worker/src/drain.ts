...
export interface DrainDeps {
  ...
  slug: string;
  /**
   * Convex canvas _id used as the Redis key namespace (canvas:{redisCanvasId}:stream).
   * After FEN-1564 the gateway keys all Redis writes by Convex _id rather than slug;
   * set this to `durable.canvasId` so the worker reads from the same namespace.
   * Defaults to `slug` when absent (backward-compat / unit tests without Convex).
   */
...
export async function drainOnce(d: DrainDeps, cursor: string): Promise<DrainOutcome> {
  const redisId = d.redisCanvasId ?? d.slug;
  const entries = await readNew(d.redis, redisId, cursor, d.maxBatch);
  if (entries.length === 0) {
  ...
  await trimStream(d.redis, redisId, batch.lastId);
