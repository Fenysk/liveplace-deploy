...
/**
 * Whether the canvas owner is currently live on Twitch (FEN-2132).
 *
 * Used by the canvas header to show the clickable live badge. Reads the same
 * `streamStatus` table the twitchLive cron writes every 60s; defaults to
 * `{ twitchLive: false }` when no canvas or profile is found.
 */
export const getCanvasLiveStatus = query({
  args: { slug: v.string() },
  returns: v.object({ twitchLive: v.boolean() }),
  handler: async (ctx, args): Promise<{ twitchLive: boolean }> => {
    const canvas = await ctx.db
      .query("canvases")
      .withIndex("by_slug", (q) => q.eq("slug", args.slug))
      .unique();
    if (!canvas) return { twitchLive: false };
    const profile = await getProfileByAuthUserId(ctx.db, canvas.ownerId);
    if (!profile?.twitchId) return { twitchLive: false };
    const ss = await ctx.db
      .query("streamStatus")
      .withIndex("by_twitchId", (q) => q.eq("twitchId", profile.twitchId))
      .unique();
    return { twitchLive: ss?.isLive ?? false };
  },
});
