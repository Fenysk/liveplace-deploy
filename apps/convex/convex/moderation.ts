...
import { gatewayPost, type DispatchResult } from "./lib/gateway";
import { authorOfTop, groupCellsAt, planDelete, planWipe, removalCells } from "./lib/moderation";
 ...
 */
export const prepareGroupDelete = internalMutation({
  args: {
    canvasId: v.id("canvases"),
    actorUserId: v.string(),
    x: v.number(),
...
async function runCellRewrite(
  ...
  const meta = await ctx.runQuery(internal.moderation.getActionMeta, { canvasId, actorUserId });
  await forceFlush(meta.slug); // mass action: flush before reading/writing state
...
export const banAndWipe = action({
  ...
  handler: async (ctx, a): Promise<ActionResult> => {
    ...
    return runCellRewrite(ctx, a.canvasId, actorUserId, () =>
      ctx.runMutation(internal.moderation.prepareBanWipe, {
        canvasId: a.canvasId,
        targetUserId: a.targetUserId,
        actorUserId,
        reason: a.reason,
...
export const deletePixels = action({
  ...
  handler: async (ctx, a): Promise<ActionResult> => {
    const actorUserId = await requireUserId(ctx);
    return runCellRewrite(ctx, a.canvasId, actorUserId, () =>
      ctx.runMutation(internal.moderation.prepareDelete, {
        canvasId: a.canvasId,
        actorUserId,
        cells: a.cells,
        reason: a.reason,
...
export const deleteGroupAt = action({
  ...
  handler: async (ctx, a): Promise<ActionResult> => {
    const actorUserId = await requireUserId(ctx);
    return runCellRewrite(ctx, a.canvasId, actorUserId, () =>
      ctx.runMutation(internal.moderation.prepareGroupDelete, {
        canvasId: a.canvasId,
        actorUserId,
        x: a.x,
        y: a.y,
...
export const restore = action({
  ...
  handler: async (ctx, a): Promise<ActionResult> => {
    ...
    return runCellRewrite(ctx, a.canvasId, actorUserId, () =>
      ctx.runMutation(internal.moderation.prepareRestore, {
        canvasId: a.canvasId,
        actorUserId,
...
export const canModerate = query({
  ...
  handler: async (ctx, a) => {
    ...
    const mod = await ctx.db
      .query("canvasModerators")
      .withIndex("by_canvas_user", (q) => q.eq("canvasId", a.canvasId).eq("userId", userId))
      .unique();
