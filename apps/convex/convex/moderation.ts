...
import { authorOfTop, groupCellsAt, planDelete, planWipe, planModeratorSync, removalCells, planPreview } from "./lib/moderation";
...
/**
 * FEN-2159 (E3-A) — read-only preview of the cells a moderation action would
 * affect. Reuses the EXACT same pure functions as the real actions (AC-B1/B2/B3
 * anti-divergence contract). NO `forceFlush` (AC-B5); the real action flushes
 * before acting — a small timing gap between preview and commit is tolerated and
 * documented (AC-B3). Bounded to `MOD_PREVIEW_MAX_CELLS`; `count`/`bbox` are
 * always computed on the full set before truncation (AC-F4). Mod-authorised.
 */
export const previewModerationTargets = query({
  args: {
    canvasId: v.id("canvases"),
    x: v.number(),
    y: v.number(),
    action: v.union(v.literal("deleteGroup"), v.literal("ban")),
  },
  returns: v.object({
    count: v.number(),
    bbox: v.union(
      v.null(),
      v.object({ minX: v.number(), minY: v.number(), maxX: v.number(), maxY: v.number() }),
    ),
    cells: v.array(v.object({ x: v.number(), y: v.number(), color: v.number() })),
    truncated: v.boolean(),
    targetResolved: v.boolean(),
  }),
  handler: async (ctx, a) => {
    const actorUserId = await requireUserId(ctx);
    await assertCanModerate(ctx, a.canvasId, actorUserId);

    if (a.action === "deleteGroup") {
      const placements = await loadPlacements(ctx, a.canvasId);
      const { cells } = groupCellsAt(placements, { x: a.x, y: a.y });
      const plans = planDelete(placements, cells);
      return { ...planPreview(plans), targetResolved: plans.length > 0 };
    }

    // ban: resolve author at anchor via DB index — same pattern as authorAt (line 933)
    const top = await ctx.db
      .query("placements")
      .withIndex("by_canvas_cell", (q) =>
        q.eq("canvasId", a.canvasId).eq("x", a.x).eq("y", a.y),
    ...
    const author = authorOfTop(top);
    if (!author) {
      return { count: 0, bbox: null, cells: [], truncated: false, targetResolved: false };
    }
    const placements = await loadPlacements(ctx, a.canvasId);
    const plans = planWipe(placements, author.userId);
    return { ...planPreview(plans), targetResolved: true };
  },
});
