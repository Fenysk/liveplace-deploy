...
export const applyFlush = internalMutation({
  ...
  handler: async (ctx, a) => {
    ...
    let maxVersion = 0;
    const inserted: Array<{ userId?: string; ts: number }> = [];
    ...
    // Advance gallery lastActivityAt to the newest fresh-insert ts (monotonic).
    // Atomic with this transaction: if applyFlush commits, the activity sort key
    // is guaranteed to reflect the drained placements (FEN-696).
    if (inserted.length > 0) {
      ...
      if (newestTs > 0) {
        ...
        if (canvas && newestTs > (canvas.lastActivityAt ?? 0)) {
          await ctx.db.patch(canvasId, { lastActivityAt: newestTs });
        }
      }
    }
