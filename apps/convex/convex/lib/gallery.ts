...
export interface GalleryItem {
  ...
  /**
   * URL of the latest durable snapshot blob, or `null` if none exists yet.
   * Used as a client-side fallback when no worker-rendered thumbnail is available:
   * the web client fetches + decodes the palette-indexed blob and renders it to a
   * <canvas> element. Anonymous-safe: Convex storage URLs are publicly readable.
   */
  ...
  lastActivityAt: number;
  /**
   * Whether the canvas owner's Twitch channel is currently live, per the last
   * Helix poll (refreshed every 60s by the twitchLive cron). Defaults to
   * `false` when unknown or on error (A7, FEN-1868).
   */
  twitchLive: boolean;
}
 ...
 *
 * `twitchLive` defaults to `false` (A7: absence/error → false, FEN-1868).
 */
export function toGalleryItem(
  ...
  return {
    ...
    latestSnapshotUrl: latestSnapshotUrl ?? null,
