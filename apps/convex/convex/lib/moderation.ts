 ...
 */
import type { ModSource } from "../schema";
...
// ─────────────────────────────────────────────────────────────────────────────
// F8.6 — Preview helper (FEN-2159): format removal plans as a query response.
// ─────────────────────────────────────────────────────────────────────────────

/** Maximum cells returned in a preview response (before truncation). */
export const MOD_PREVIEW_MAX_CELLS = 4096;

/** A preview cell: the top visible pixel at (x,y) — `color` is `removedColor` (never 0). */
...
/** Bounding box computed over the complete preview set (before the cap). */
...
/**
 * Format removal plans as a preview response. `count` and `bbox` are exact
 * over the FULL set before capping to `maxCells`; cells are row-major (callers
 * `planDelete` and `planWipe` already call `sortPlans`). Each cell's `color`
 * is the top-visible index (`removedColor`) — never 0, since both plan helpers
 * skip erased tops.
 */
export function planPreview(
  plans: ReadonlyArray<RemovalPlan>,
  maxCells: number = MOD_PREVIEW_MAX_CELLS,
): { count: number; bbox: PreviewBbox | null; cells: PreviewCell[]; truncated: boolean } {
  const count = plans.length;
  let bbox: PreviewBbox | null = null;
  if (count > 0) {
  ...
  const truncated = count > maxCells;
  const cells: PreviewCell[] = plans.slice(0, maxCells).map((p) => ({
    x: p.x,
    y: p.y,
    color: p.removedColor,
  }));
  return { count, bbox, cells, truncated };
}

// ─────────────────────────────────────────────────────────────────────────────
// F8.5 — Twitch moderator roster diffing (CA5).
// ─────────────────────────────────────────────────────────────────────────────

/** Minimal moderator row for roster diffing (subset of `canvasModerators` Doc). */
...
/** An incoming Twitch moderator entry from Helix. */
export interface IncomingMod {
  twitchId: string;
  login?: string;
  displayName?: string;
}

/** What `applyTwitchSync` must do: upsert these + deactivate these twitchIds. */
export interface ModSyncPlan {
  toUpsert: IncomingMod[];
  toDeactivate: string[];
}

/**
 * CA5 — diff the incoming Twitch roster against the existing rows and return
 * the minimal sync plan: every incoming mod is in `toUpsert` (insert-or-patch),
 * every previously-active `twitch_sync` row no longer present is in
 * `toDeactivate`. `source === "manual"` rows are never touched by sync.
 */
export function planModeratorSync(
  existingRows: ReadonlyArray<ModRow>,
  incomingMods: ReadonlyArray<IncomingMod>,
): ModSyncPlan {
  const present = new Set(incomingMods.map((m) => m.twitchId));
  const toDeactivate: string[] = [];
  for (const row of existingRows) {
    if (row.source === "twitch_sync" && row.active && !present.has(row.twitchId)) {
      toDeactivate.push(row.twitchId);
    }
  }
  return { toUpsert: [...incomingMods], toDeactivate };
}
