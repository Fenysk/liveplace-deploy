...
export interface GaugeProps {
  ...
  hideCount?: boolean;
  /**
   * cooldown mode — a short glyph rendered in the ring's centre disc (FEN-1482).
   * Turns the bare "Xs" into "+N in Xs" without eating row width: e.g. `"+1"`
   * makes the ring itself read as "+1 pixel", the seconds stay alongside, and
   * the sibling reserve bar keeps its full length. A numeral ⇒ no i18n. Omitted
   * ⇒ the plain white disc (unchanged).
   */
  stepLabel?: string;
  /**
  ...
  if (mode === "cooldown") {
    return (
      <span className="ui-gauge" data-phase={phase ?? undefined}>
        ...
        >
          {stepLabel && (
            <span className="ui-gauge__inner">
              <span className="ui-gauge__ringnum">{stepLabel}</span>
            </span>
          )}
        </span>
