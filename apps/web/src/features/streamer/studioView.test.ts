...
test("BROADCAST_STEP_KEYS are the three numbered OBS steps, in order", () => {
  assert.deepEqual(
    [...BROADCAST_STEP_KEYS],
    ["studio.broadcast.step1", "studio.broadcast.step2", "studio.broadcast.step3"],
  );
});
...
test("buildCreateArgs carries only the fields the streamer set", () => {
  ...
  assert.deepEqual(buildCreateArgs({ name: "Neon", sizeKey: "large" }), {
    title: "Neon",
    width: 250,
    height: 250,
  });
  assert.deepEqual(buildCreateArgs({ name: "", isPublic: true }), { isPublic: true });
  assert.deepEqual(buildCreateArgs({ name: "", paletteId: "pal_1" }), { paletteId: "pal_1" });
  // A null palette (explicit "default") is omitted, not forwarded.
  assert.deepEqual(buildCreateArgs({ name: "", paletteId: null }), {});
});

test("size presets: default key resolves, medium mirrors the backend default 100", () => {
  assert.equal(DEFAULT_SIZE_KEY, "medium");
  assert.equal(sizePreset("medium").dimension, 100);
  assert.equal(sizePreset("small").dimension, 50);
  assert.equal(sizePreset("large").dimension, 250);
  // Every preset stays within the canvasRules MIN/MAX dimension bounds (16…512).
  for (const p of SIZE_PRESETS) {
    assert.ok(p.dimension >= 16 && p.dimension <= 512, `${p.key} in bounds`);
  }
});
