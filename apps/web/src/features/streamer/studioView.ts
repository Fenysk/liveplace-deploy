 ...
 * functions (`canvases.*`), hand the result here, and render the returned
 * descriptor. i18n keys are RETURNED, never resolved here, so this stays
 * locale-agnostic (same convention as galleryView.ts).
 *
 * Spec: FEN-83 ux-spec §D6 flows S1/S2, §D7 WF-5/6/7, persona Sam (F9/F10/F11).
 * Backend contract: apps/convex/convex/canvases.ts.
 */
...
export interface SizePreset {
  key: SizeKey;
  /** Square canvas edge in cells (within canvasRules MIN/MAX = 10…512 after 3a). */
  dimension: number;
...
/**
 * Size presets with their consequence copy (F9). `xs` (10×10) mirrors the
 * backend default (`canvasRules.DEFAULT_DIMENSION`) so the minimal path — create
 * without touching advanced options — and an explicit "xs" pick agree.
 */
export const SIZE_PRESETS: readonly SizePreset[] = [
  { key: "xs", dimension: 10, labelKey: "studio.size.xs", hintKey: "studio.size.xs.hint" },
  { key: "s", dimension: 20, labelKey: "studio.size.s", hintKey: "studio.size.s.hint" },
  { key: "m", dimension: 50, labelKey: "studio.size.m", hintKey: "studio.size.m.hint" },
  { key: "l", dimension: 100, labelKey: "studio.size.l", hintKey: "studio.size.l.hint" },
];

/** The default size key (matches the backend default dimension). */
export const DEFAULT_SIZE_KEY: SizeKey = "xs";

/** Resolve a size key to its preset (falls back to the default). */
export function sizePreset(key: SizeKey): SizePreset {
  return SIZE_PRESETS.find((p) => p.key === key) ?? SIZE_PRESETS[0]!;
}
 ...
 * options untouched, the args are `{}`/`{title}` and the server fills in 10×10,
 * placement-open, private (flow S1 "défauts pré-remplis").
 */
