...
export function CrisisSelector(props: CrisisSelectorProps): React.ReactElement {
  ...
  useEffect(() => {
    ...
    // `paintAim`/`pick` are stable per-render closures captured intentionally; this
    // effect must connect the renderer/socket ONCE per slug, not re-run on every
    // render, so they are deliberately excluded from the dep array.
