...
import { usePixelInspect, type ModAction } from "./usePixelInspect.js";
...
const getCanvasLiveStatusRef = api.canvases.getCanvasLiveStatus;
...
export interface CanvasViewProps {
  ...
  /**
   * Render prop for the live moderation preview (FEN-2156 E3-C / AC-F8).
   * Threaded from {@link CanvasViewLive} (which owns the auth-gated canvas id)
   * through to {@link PixelInfoPanel}, keeping PixelInfoPanel Convex-free.
   * Absent ⇒ no preview rendered (behaviour unchanged).
   */
  renderModPreview?: (args: { x: number; y: number; action: ModAction }) => ReactNode;
}
...
export function CanvasView({
  ...
  const liveStatus = useQuery(getCanvasLiveStatusRef, slug ? { slug } : "skip");
      ...
      el,
      {
        ...
        onSpaceHold: (held) => {
          applySpaceHold(held, {
            ...
            hoverRef,
            stageCell,
            colorAt: (x, y) => rendererRef.current?.colorAt(x, y) ?? -1,
  ...
  return (
    <div className="lp-app" data-eyedropper-mode={eyedropperMode ? "on" : undefined}>
      ...
      <CanvasChrome
        ...
        twitchLive={liveStatus?.twitchLive ?? false}
      ...
      <BottomSheet
        ...
        headerLeft={showHandle && hasFinePointer ? (
          <div className="lp-sc-row" role="group" aria-label={t("canvas.shortcuts.title")}>
            {drawing ? (
              <>
              ...
              <>
                <ShortcutItem keyLabel={t("canvas.shortcuts.key.enter")} role={t("canvas.shortcuts.role.enter.draw")} />
                <ShortcutItem keyLabel={t("canvas.shortcuts.key.esc")} role={t("canvas.shortcuts.role.esc")} />
              </>
            )}
          </div>
      ...
      >
        ...
        {/* FEN-2177: idle keyboard hint — fine-pointer (desktop) only, no active mode.
            Shows "Entrée — Dessiner" so keyboard users know how to start placing. */}
        {hasFinePointer && !hasActiveMode && (
          <div className="lp-sc-row lp-sc-idle-hint" role="group" aria-label={t("canvas.shortcuts.title")}>
            <ShortcutItem keyLabel={t("canvas.shortcuts.key.enter")} role={t("canvas.shortcuts.role.enter.draw")} />
          </div>
        )}
        ...
        {pixelInfo && (
          <PixelInfoPanel
            ...
            renderModPreview={renderModPreview}
            t={t}
          />
        )}
