...
// ─── Assertion 6 — FEN-2151 : ignore pixels déjà à la couleur sélectionnée ──
// Espace+hover sur un pixel dont la couleur board == couleur palette → ignoré.
// Sur un pixel de couleur différente → stagé normalement.
// Avec gomme active → le skip ne s'applique pas (la gomme écrase toujours).

test("C5-A6 — Espace+hover : pixel déjà à la couleur sélectionnée est ignoré (FEN-2151)", () => {
  const spacePaintingRef = { current: false };
  const drawingRef = { current: true };
  const hoverRef: { current: { x: number; y: number } | null } = { current: null };
  const sel = new BatchSelection(5);
  const stageCell = (x: number, y: number, _opts?: { onlyAdd?: boolean }): void => {
    sel.apply(x, y, 1);
  };

  const selectedColor = { current: 1 }; // palette index 1
  const erasingRef = { current: false };
  // (2,0) est déjà à la couleur 1 sur le board ; (0,0) et (1,0) ont la couleur 0.
  const boardColors: Record<string, number> = { "2,0": 1 };
  const colorAt = (x: number, y: number): number => boardColors[`${x},${y}`] ?? 0;

  const ctx = { spacePaintingRef, drawingRef, hoverRef, stageCell, colorAt, selectedColor, erasingRef };
  spacePaintingRef.current = true;

  // Pixel (0,0) — couleur board 0 ≠ 1 → stagé
  applyHoverSpacePaint({ x: 0, y: 0 }, ctx);
  assert.equal(sel.count, 1, "(0,0) stagé : couleur différente");

  // Pixel (2,0) — couleur board 1 == couleur sélectionnée 1 → ignoré
  applyHoverSpacePaint({ x: 2, y: 0 }, ctx);
  assert.equal(sel.count, 1, "(2,0) ignoré : déjà à la couleur sélectionnée");
  assert.ok(!sel.has(2, 0), "(2,0) absent de la sélection");

  // Pixel (1,0) — couleur board 0 ≠ 1 → stagé
  applyHoverSpacePaint({ x: 1, y: 0 }, ctx);
  assert.equal(sel.count, 2, "(1,0) stagé : couleur différente");

  // Gomme active : (2,0) doit être stagé même si couleur board == selectedColor
  erasingRef.current = true;
  applyHoverSpacePaint({ x: 2, y: 0 }, ctx);
  assert.equal(sel.count, 3, "(2,0) stagé avec la gomme active même si même couleur");
  assert.ok(sel.has(2, 0), "(2,0) présent : gomme bypass le skip");
});

// ─── FEN-2152 — CA1/CA4 : guard couleur sur premier appui (applySpaceKeyDown + applySpaceHold) ───

function makeColorGuardCtx(boardColors: Record<string, number>, selectedColorIdx: number, erasing = false) {
  const spacePaintingRef = { current: false };
  const drawingRef = { current: true };
  const hoverRef: { current: { x: number; y: number } | null } = { current: null };
  const sel = new BatchSelection(5);
  const stageCell = (x: number, y: number, _opts?: { onlyAdd?: boolean }): void => {
    sel.apply(x, y, selectedColorIdx);
  };
  const selectedColor = { current: selectedColorIdx };
  const erasingRef = { current: erasing };
  const colorAt = (x: number, y: number): number => boardColors[`${x},${y}`] ?? 0;
  const ctx: SpaceHoldCtx = { spacePaintingRef, drawingRef, hoverRef, stageCell, colorAt, selectedColor, erasingRef };
  return { ctx, sel };
}

test("CA1 — applySpaceKeyDown : premier appui sur pixel déjà à la couleur → ignoré", () => {
  // Board: (3,3) a la couleur 2 ; selectedColor = 2 → doit être ignoré.
  const { ctx, sel } = makeColorGuardCtx({ "3,3": 2 }, 2);
  ctx.hoverRef.current = { x: 3, y: 3 };

  applySpaceKeyDown(false, ctx);

  assert.equal(sel.count, 0, "pixel (3,3) ignoré : même couleur que la palette");
  assert.ok(ctx.spacePaintingRef.current, "spacePaintingRef armé malgré le skip");
});

test("CA1 — applySpaceHold : premier appui sur pixel déjà à la couleur → ignoré", () => {
  const { ctx, sel } = makeColorGuardCtx({ "3,3": 2 }, 2);
  ctx.hoverRef.current = { x: 3, y: 3 };

  applySpaceHold(true, ctx);

  assert.equal(sel.count, 0, "pixel (3,3) ignoré : même couleur que la palette");
  assert.ok(ctx.spacePaintingRef.current, "spacePaintingRef armé malgré le skip");
});

test("CA1 — applySpaceKeyDown : premier appui sur pixel de couleur différente → stagé", () => {
  // Board: (3,3) a la couleur 0 ; selectedColor = 2 → doit être stagé.
  const { ctx, sel } = makeColorGuardCtx({}, 2);
  ctx.hoverRef.current = { x: 3, y: 3 };

  applySpaceKeyDown(false, ctx);

  assert.equal(sel.count, 1, "pixel (3,3) stagé : couleur différente");
});

test("CA4 — applySpaceKeyDown : gomme active, même couleur → stagé (bypass)", () => {
  const { ctx, sel } = makeColorGuardCtx({ "3,3": 2 }, 2, /* erasing */ true);
  ctx.hoverRef.current = { x: 3, y: 3 };

  applySpaceKeyDown(false, ctx);

  assert.equal(sel.count, 1, "pixel (3,3) stagé : gomme active bypass le guard");
});

test("CA4 — applySpaceHold : gomme active, même couleur → stagé (bypass)", () => {
  const { ctx, sel } = makeColorGuardCtx({ "3,3": 2 }, 2, /* erasing */ true);
  ctx.hoverRef.current = { x: 3, y: 3 };
