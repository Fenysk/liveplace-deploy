...
import { LiveModPreview } from "./LiveModPreview.js";
...
export function CanvasViewLive({ slug }: CanvasViewLiveProps): React.ReactElement | null {
  ...
  // FEN-2156 E3-C: render prop for live mod preview, created with the auth-gated
  // canvasId so the query runs only once the Convex backend has confirmed auth.
  // Returns null when canvasId is null (pre-auth), skipping the subscription.
  const renderModPreview = useCallback(
    ({ x, y, action }: { x: number; y: number; action: ModAction }): ReactNode =>
      canvasId ? (
        <LiveModPreview canvasId={canvasId} x={x} y={y} action={action} t={t} />
      ) : null,
    [canvasId, t],
  );
