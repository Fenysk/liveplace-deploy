...
test("first account-requiring interaction triggers consent back to the same canvas", () => {
  for (const i of ACCOUNT_GATED) {
    assert.deepEqual(
      gateInteraction(i, false, { slug: "pixelwar" }),
      { kind: "consent", callbackURL: "/c/pixelwar" },
...
test("callbackURL returns to the named canonical /c/{slug} path", () => {
  assert.equal(canvasCallbackURL("pixelwar"), "/c/pixelwar");
  // Stable even when consent fires from a non-canonical path.
  assert.equal(canvasCallbackURL("pixelwar", "/canvas"), "/c/pixelwar");
  // Slug is URL-encoded so unusual slugs survive the redirect.
  assert.equal(canvasCallbackURL("été 2026"), `/c/${encodeURIComponent("été 2026")}`);
});
