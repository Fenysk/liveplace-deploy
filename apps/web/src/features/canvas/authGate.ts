...
export type GateDecision =
  | { kind: "proceed" }
  | { kind: "consent"; callbackURL: string; errorCallbackURL?: string };
...
export interface GateOptions {
  ...
  currentPath?: string;
  /** AC10: URL to redirect to on OAuth error. Propagated into consent verbatim. */
...
export function gateInteraction(
  ...
  return {
    ...
    ...(options.errorCallbackURL != null ? { errorCallbackURL: options.errorCallbackURL } : {}),
  };
}
