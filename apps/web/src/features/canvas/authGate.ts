 ...
 *
 * A named canvas (`slug`) always resolves to its canonical `/c/{slug}` path so
 * the return is stable even if the consent was triggered from `/canvas` or a
 * query-string variant. The default canvas (no slug) returns to the current
 * path verbatim (`/` or `/canvas`).
 */
