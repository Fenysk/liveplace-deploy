...
export interface CanvasNetHandlers {
  ...
  /**
   * The canvas geometry changed (resize) — re-render at new dimensions immediately.
   * Existing pixels are preserved; new cells are empty. Fired by the server
   * `dimsChanged` frame (FEN-1790, ADDITIVE & non-breaking).
   */
...
export class CanvasNetClient {
  ...
  private onMessage(data: unknown): void {
    ...
    switch (msg.t) {
      ...
      case "moderationEvent":
        ...
        break;
      case "dimsChanged":
        // Canvas resized by the owner — re-render at new dimensions without a
        // reconnect (FEN-1790, ADDITIVE non-breaking frame).
        this.handlers.onDimsChanged?.(msg.width, msg.height);
        break;
      case "pong":
        break;
    }
  ...
  /** Ask the gateway to re-push the current gauge as a `{ t: "gauge" }` frame.
   *  No-op if the socket is not open. Emitted at local refill expiry and on focus/visibility restore. */
  requestGaugePeek(): void {
    this.sendRaw({ t: "gaugePeek" });
  }
