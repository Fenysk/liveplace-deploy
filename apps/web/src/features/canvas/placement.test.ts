...
test("onCommitted fires with erased=false on a paint ack", () => {
  ...
  ctrl.handle({ t: "ack", seq: 0, cid: m.cid, charges: 10, max: 10, cooldownUntil: 0 });
...
test("onCommitted fires per-op (multiple sequential placements)", () => {
  ...
  ctrl.handle({ t: "ack", seq: 0, cid: a.cid, charges: 9, max: 10, cooldownUntil: 0 });
  ctrl.handle({ t: "ack", seq: 0, cid: b.cid, charges: 8, max: 10, cooldownUntil: 0 });
