...
test("setBottomInset: fit uses visible height for scale; centres in visible zone above dock (AC-R2-2/3, FEN-383, FEN-702)", () => {
  const v = new ViewTransform();
  ...
  // ty centres the board in the available zone (between topInset=0 and visH) — no
  // large blank band at the top (FEN-702 F1 D3). With topInset=0, availH=visH.
  const expectedTy = Math.max(0, (visH - ch) / 2);
  assert.ok(Math.abs(v.ty - expectedTy) < 1e-6, `ty=${v.ty} should equal centre-in-visible-zone=${expectedTy}`);
...
test("FEN-1322: desktop sidebar — zero bottomInset produces correct fit, large bottomInset produces tiny scale", () => {
  // Correct desktop path: the sidebar sits beside the canvas (not below it),
  // so bottomInset must be 0. board should appear at a comfortable scale.
  const correct = new ViewTransform();
  correct.setBoard(512, 512);
  correct.setViewport(1280 * 2, 964 * 2); // 1280×964 CSS px at dpr=2
  correct.setBottomInset(0);
  correct.fit();
  assert.ok(correct.scale > 1, `desktop fit scale ${correct.scale} should be > 1 (visible board)`);
  assert.ok(Math.abs(correct.ty - (964 * 2 - 512 * correct.scale) / 2) < 1e-6, "board should be vertically centred");

  // Bug scenario: sidebar height (≈ full viewport height) wrongly passed as bottomInset.
  // visH collapses to ~0, fitScale → 1/512 ≈ 0.002, board appears tiny at the top.
  const buggy = new ViewTransform();
  buggy.setBoard(512, 512);
  buggy.setViewport(1280 * 2, 964 * 2);
  buggy.setBottomInset(964 * 2); // sidebar height == viewport height → visH ≈ 0
  buggy.fit();
  assert.ok(buggy.scale < 0.01, `buggy scale ${buggy.scale} should be near-zero`);
  assert.ok(buggy.ty <= 1, `buggy ty ${buggy.ty} should be at the top`);
  assert.ok(correct.scale > buggy.scale * 100, "correct scale should dwarf the buggy scale");
});
