...
/**
 * Resolve the `permission` input to {@link derivePlaceState} from the route slug
 * and the live `canPlace` query result.
 *
 * The DEFAULT canvas (`/`, slug === null) has NO Convex doc and NO `canPlace`
 * contract: it is the always-on hero and the gateway is the placement authority
 * (gauge / cooldown). It is therefore implicitly OPEN — return `{ allowed: true }`
 * so the unified indicator leans on auth + gauge. Returning `undefined` there
 * (as if the query were still loading) pins derivePlaceState's loading gate
 * (`permission === undefined`) ON forever, so the landing canvas never leaves
 * "Loading the canvas…" even with a valid gauge — making placement impossible on
 * it (FEN-277, exposed once FEN-267 restored the gauge wiring).
 *
 * A SLUGGED canvas forwards the live query result verbatim: `undefined` while it
 * loads is a genuine "permission unknown" that should hold the indicator at
 * "loading" until the contract resolves.
 */
export function resolvePermission(
  slug: string | null,
  result: Permission | undefined,
): Permission | undefined {
  return slug ? result : { allowed: true };
}
...
export interface PlaceStateInput {
  ...
  /**
   * True while `useSession()` is still resolving. When pending the session is
   * null even for a connected user, so we suppress the `signedOut` flash and
   * return `loading` instead (AP-12).
   */
  authPending?: boolean;
...
export function derivePlaceState(input: PlaceStateInput): PlaceState {
  ...
  // Auth pending: session is transiently null while useSession() resolves.
  // Return loading instead of signedOut to suppress the flash (AP-12).
  if (input.authPending && !authenticated) {
  ...
  if (gaugeEmpty(gauge)) {
    return {
      ...
      params: { seconds: secondsUntil(gauge.cooldownUntil, now), max: gauge.max },
