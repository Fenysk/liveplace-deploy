...
export interface LiveModerationSourceDeps {
  ...
  deleteGroupAt: (args: { canvasId: Id<"canvases">; x: number; y: number }) => Promise<CellActionResult>;
  /** Bound `moderation.authorAt` query — resolves the ban target at a cell. */
  authorAt: (args: { canvasId: Id<"canvases">; x: number; y: number }) => Promise<AuthorAtResult | null>;
  /** Bound `moderation.banAndWipe` action (S8.5). */
  banAndWipe: (args: { canvasId: Id<"canvases">; targetUserId: string }) => Promise<CellActionResult>;
}
