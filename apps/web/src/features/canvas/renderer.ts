 ...
 */
import { PALETTE, PALETTE_SIZE, binaryOpcode, OP_DELTA, OP_SNAPSHOT } from "@canvas/protocol";
import { BoardBuffer } from "./boardBuffer.js";
...
export interface RendererHooks {
  ...
  onEraserToggle?: () => void;
  /** Keyboard I — eyedropper: pick the colour under the cursor (FEN-615 G8 / FEN-1732). */
  onEyedropper?: () => void;
  /**
  ...
  /**
   * Right-button stationary click (desktop) on cell (x,y) — deselect path
   * (FEN-1578). Only fires when no drag threshold was crossed. Never fires on
   * touch (right-button is mouse/pen only).
   */
  onRightTap?: (x: number, y: number, pointerType: string) => void;
  /**
   * Right-button drag: fired on each pointermove while the right button is
   * held past the drag threshold (FEN-1578). `cell` is the target cell, or
   * null when the pointer is off-canvas. Used for continuous deselect-by-drag
   * on desktop. The canvas is NOT panned during a right-drag.
   */
  onRightHover?: (cell: { x: number; y: number } | null) => void;
  /**
   * Space key held/released — single-source notification so CanvasView can arm
   * its drag-paint flag before the renderer's ensureCursor clobbers hoverRef.
   * `held = true` on first keydown (not repeat), `false` on keyup or window blur.
   */
...
export class CanvasRenderer {
  private readonly buf = new BoardBuffer();
  private image?: ImageData;
  /** DOM-backed Uint32Array view of `image.data.buffer` — synced from `buf.imageView`. */
  private domView?: Uint32Array;
  ...
  private downButton = 0; // mouse button that initiated the current gesture (FEN-1578)
  ...
  loadSnapshot(buf: ArrayBuffer): number {
    const firstLoad = !this.buf.loaded;
    const seq = this.buf.loadSnapshot(buf);
    this.syncBoardDom(firstLoad);
    return seq;
  }

  /** Sync the DOM offscreen canvas from `buf` after a snapshot load or resize. */
  private syncBoardDom(firstLoad: boolean): void {
    this.offscreen.width = this.buf.width;
    this.offscreen.height = this.buf.height;
    this.image = this.octx.createImageData(this.buf.width, this.buf.height);
    this.domView = new Uint32Array(this.image.data.buffer);
    this.domView.set(this.buf.imageView);
    this.octx.putImageData(this.image, 0, 0);
    this.view.setBoard(this.buf.width, this.buf.height);
    this.recenter();
  ...
  private panToLiveCentroid(): void {
    ...
    for (let i = 0; i < this.buf.pixels.length; i++) {
      if (this.buf.pixels[i] !== 0) {
        sumX += i % this.buf.width;
        sumY += (i / this.buf.width) | 0;
        count++;
      }
    }
    if (count === 0) return;
  ...
  applyDelta(buf: ArrayBuffer): number {
    return this.buf.applyDelta(buf, (x, y, idx) => {
      if (this.domView && this.image) {
        this.domView[idx] = this.buf.imageView[idx]!;
  ...
  get appliedSeq(): number {
    return this.buf.appliedSeq;
  }

  /**
   * Handle a `dimsChanged` WS frame (FEN-1790): expand the board to new
   * dimensions, preserving existing pixels, zero-filling the new cells.
   * Does NOT recenter (keeps the user's current zoom/pan position).
   */
  resizeTo(newWidth: number, newHeight: number): void {
    if (!this.buf.loaded) return;
    if (newWidth === this.buf.width && newHeight === this.buf.height) return;

    this.buf.resizeTo(newWidth, newHeight);

    this.offscreen.width = newWidth;
    this.offscreen.height = newHeight;
    this.image = this.octx.createImageData(newWidth, newHeight);
    this.domView = new Uint32Array(this.image.data.buffer);
    this.domView.set(this.buf.imageView);
    this.octx.putImageData(this.image, 0, 0);

    this.view.setBoard(newWidth, newHeight);
    this.dirty = true;
  }
  ...
  setPixel(x: number, y: number, color: number): void {
    if (!this.image || !this.domView) return;
    if (x < 0 || y < 0 || x >= this.buf.width || y >= this.buf.height) return;
    this.buf.setPixel(x, y, color);
    const idx = y * this.buf.width + x;
    this.domView[idx] = this.buf.imageView[idx]!;
  ...
  toCell(clientX: number, clientY: number): { x: number; y: number } | null {
    if (!this.buf.loaded) return null;
  ...
  private draw(): void {
    ...
    ctx.rect(
      ...
      Math.ceil(this.buf.width * this.view.scale),
      Math.ceil(this.buf.height * this.view.scale),
    );
    ctx.clip();
   ...
   *
   * FEN-1711: both rings are drawn STRICTLY OUTSIDE the board — no canvas pixel
   * is ever covered. Each ring = 1 canvas-pixel wide (lineWidth = scale s), so
   * the border scales with zoom exactly like the pixels do.
   * Layout from board edge outward: [board] | white (s px) | black (s px)
   */
  private drawBoardBorder(): void {
    ...
    const bw = Math.round(this.buf.width * s);
    const bh = Math.round(this.buf.height * s);
    ...
    // 1 canvas-pixel per ring. strokeRect(x,y,w,h) centres the stroke on the
    // rect perimeter, so rect expanded by s/2 → inner stroke edge flush with bx.
    ctx.lineWidth = s;

    // White ring (immediately outside board, legible against dark board content).
    ctx.strokeStyle = "rgba(255,255,255,0.9)";
    ctx.strokeRect(bx - s / 2, by - s / 2, bw + s, bh + s);

    // Black ring (outermost, legible against the neutral/checkerboard field).
    ctx.strokeStyle = "rgba(0,0,0,0.7)";
    ctx.strokeRect(bx - s * 1.5, by - s * 1.5, bw + s * 3, bh + s * 3);
  }
  ...
  private drawGrid(): void {
    ...
    const x1 = Math.min(this.buf.width, Math.ceil((cw - this.view.tx) / s));
    const y0 = Math.max(0, Math.floor((0 - this.view.ty) / s));
    const y1 = Math.min(this.buf.height, Math.ceil((ch - this.view.ty) / s));
    ctx.beginPath();
    for (let x = x0; x <= x1; x++) {
  ...
  private bindInput(): void {
    ...
    // FEN-1578: suppress the browser context menu so right-click is the deselect
    // gesture rather than opening a native popup on the canvas element.
  ...
  private onPointerDown = (e: PointerEvent): void => {
    ...
    if (wasIdle) {
      ...
      this.downButton = e.button; // 0=left, 1=middle, 2=right (FEN-1578)
  ...
  private onPointerMove = (e: PointerEvent): void => {
    ...
    if (this.moved) {
      if (this.downButton === 2) {
        // Right-drag (FEN-1578): don't pan — fire onRightHover for continuous deselect.
        this.hooks.onRightHover?.(this.toCell(e.clientX, e.clientY));
      } else {
  ...
  private onPointerUp = (e: PointerEvent): void => {
    ...
    if (wasSingle && !this.moved) {
      ...
      if (cell) {
        if (this.downButton === 2) {
          // FEN-1578: right-click fires its own hook (deselect path on desktop).
          this.hooks.onRightTap?.(cell.x, cell.y, this.downType);
        } else {
  ...
  // FEN-1578: suppress the browser context menu so right-click drives deselect
  // instead of opening a native popup over the canvas element.
  ...
  private onKeyDown = (e: KeyboardEvent): void => {
    if (!this.buf.loaded || e.altKey) return;
    ...
    switch (e.key) {
      ...
      case "Spacebar":
        this.spaceHeld = true;
        if (!e.repeat) this.hooks.onSpaceHold?.(true);
        break;
      case "Escape":
        this.hooks.onCancel?.();
        break;
      ...
      // !e.repeat guard: toggling on every keydown repeat would flip the tool
      // state ~30×/sec, making it unreliable (FEN-2014).
      case "e":
      case "E":
        if (!e.ctrlKey && !e.metaKey && !e.repeat) this.hooks.onEraserToggle?.();
        else return;
        break;
      case "i":
      case "I":
        if (!e.ctrlKey && !e.metaKey && !e.repeat) this.hooks.onEyedropper?.();
        else return;
        break;
      case "g":
      case "G":
  ...
  private ensureCursor(): { x: number; y: number } {
    if (!this.cursor || !this.inBoard(this.cursor.x, this.cursor.y)) {
      ...
      this.cursor = mid ?? { x: Math.floor(this.buf.width / 2), y: Math.floor(this.buf.height / 2) };
      this.syncCursor();
    }
    return this.cursor;
  }
  ...
  private moveCursor(dx: number, dy: number): void {
    ...
    if (fresh) return;
    const nx = Math.min(this.buf.width - 1, Math.max(0, cur.x + dx));
    const ny = Math.min(this.buf.height - 1, Math.max(0, cur.y + dy));
  ...
  private inBoard(x: number, y: number): boolean {
    return x >= 0 && y >= 0 && x < this.buf.width && y < this.buf.height;
  }
}
