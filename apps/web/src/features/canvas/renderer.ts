...
export class CanvasRenderer {
  ...
  // Top overlay inset (topbar height) in device pixels — clamped by drawBoardBorder
  // so the border is never drawn behind the topbar in cover mode (FEN-470).
  private topInset = 0;
   ...
   * Call whenever the topbar height changes. Used by `drawBoardBorder` to push the
   * top border below the topbar in both fit and cover modes (FEN-470).
   */
  setTopInset(cssPx: number): void {
    this.topInset = Math.round(cssPx * this.dpr);
    this.dirty = true;
  }
   ...
   *
   * Uses identity (device-pixel) coordinates so the stroke is a constant visual
   * thickness regardless of zoom (AC-3, AC-5). The board position follows
   * view.tx / view.ty so pan moves the border with the canvas exactly (AC-3).
   *
  ...
  private drawBoardBorder(): void {
    ...
    // Clamp to the visible viewport: in cover mode (mobile) the board overflows
    // the canvas buffer — strokes at bx<0 or bx+bw>canvas.width are never
    // rendered. Clamping to the board∩viewport intersection ensures all four
    // visible edges appear. The topInset clamp also pushes the top border below
    // the topbar so it is not painted behind the UI chrome (FEN-470).
    const left   = Math.max(bx, 0);
    const top    = Math.max(by, this.topInset);
    const right  = Math.min(bx + bw, this.canvas.width);
    const bottom = Math.min(by + bh, this.canvas.height);
    if (right <= left || bottom <= top) return;
    const w = right - left;
    const h = bottom - top;

    // Dark outer ring (3 px): stroke path centred 1.5 px inside the clamped
    // rect so the outer edge lands at the board/viewport boundary.
    ctx.lineWidth = 3;
