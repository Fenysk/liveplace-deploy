...
export class ViewTransform {
  ...
  /**
   * Contain (fit-to-screen) scale: the whole board fits in the VISIBLE viewport
   * (viewport − bottomInset). Always uses Math.min regardless of cover mode so
   * the user can zoom out to see the full fresco (AC-R2-3, FEN-370).
   */
  private fitScale(): number {
    if (!this.width || !this.height || !this.viewW || !this.viewH) return 1;
    return Math.min(this.viewW / this.width, this.visH / this.height) * FIT_MARGIN;
  }

  /**
   * Fit the whole board in the visible viewport (viewport − bottomInset), centred.
   * This is the "see the whole fresco" action driven by the ⊡ zoom control (AC-R2-3).
   * Scale is set to fitScale (contain); minScale floor drops to DEEP_ZOOM_FACTOR × fit
   * so the user can keep zooming out past fit (AC1, FEN-383).
   */
  fit(): void {
    ...
    // Prefer centering in the FULL viewport (viewH) so the board appears visually
    // centred on screen. Fall back to just-above-dock (visH - ch) when the board
    // is too tall to fit in the upper half of the screen — in that case, the dock
    // forces the board upward and there is no fully-centred solution (FEN-383).
    this.ty = Math.min((this.viewH - ch) / 2, this.visH - ch);
  }
