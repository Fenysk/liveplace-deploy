...
test("refill with an armed cell ⇒ ready to fire in one gesture", () => {
  ...
  // FEN-418 A3: the bar gauge signals recharge visually; the "Rechargé" text
  // is suppressed so the primary CTA "Poser X pixels" is the only call to action.
  assert.equal(v.messageKey, null);
});
...
test("waiting (cooling, nothing armed) emits no message text — ring carries the wait (FEN-899)", () => {
  const v = view({ charges: 0, staged: 0 });
  assert.equal(v.phase, "waiting");
  // FEN-899: the "Vise ta prochaine case" phrase is removed; the ring timer
  // provides the visual wait cue. No message text for the waiting phase.
  assert.equal(v.messageKey, null);
...
test("every non-null phase key exists in BOTH catalogs (FR/EN parity)", () => {
  ...
  // FEN-418 A3: refilledArmed returns null. FEN-899: waiting also returns null.
  // Only "armed" emits a non-null key now.
