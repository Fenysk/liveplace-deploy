...
test("authPending: session null while useSession() resolves → loading, not signedOut flash (AP-12)", () => {
  const s = derivePlaceState(base({ authenticated: false, authPending: true, gauge: null }));
  assert.equal(s.kind, "loading");
  assert.equal(s.canPlace, false);
  assert.equal(s.messageKey, "canvas.state.loading");
});

test("authPending: once resolved authenticated=true, authPending has no effect", () => {
  const s = derivePlaceState(base({ authenticated: true, authPending: false }));
  assert.equal(s.kind, "ready");
});

test("authPending: truly anonymous (authPending=false) still returns signedOut", () => {
  const s = derivePlaceState(base({ authenticated: false, authPending: false, gauge: null }));
  assert.equal(s.kind, "signedOut");
});
...
// FEN-277 regression: the default canvas (slug null) has no canPlace contract,
// so resolvePermission must hand derivePlaceState a permissive "allowed" — NOT
// `undefined`, which pins the loading gate ON forever and makes the landing
// canvas unplaceable despite a valid gauge.
test("resolvePermission: default canvas (slug null) → implicitly allowed, never loading", () => {
  // Even with no live query result, the default canvas is open.
  assert.deepEqual(resolvePermission(null, undefined), { allowed: true });
  // And that permission carries a signed-in, gauged viewer all the way to ready
  // (the exact path the landing canvas was stuck before "loading" of, FEN-277).
  const s = derivePlaceState(base({ permission: resolvePermission(null, undefined) }));
  assert.equal(s.kind, "ready");
  assert.equal(s.canPlace, true);
});

test("resolvePermission: slugged canvas forwards the live result (undefined = loading)", () => {
  // In flight → undefined passes through → genuine loading (held until canPlace resolves).
  assert.equal(resolvePermission("default", undefined), undefined);
  assert.equal(derivePlaceState(base({ permission: resolvePermission("default", undefined) })).kind, "loading");
  // A resolved deny is forwarded verbatim so the canvas-level reason still wins.
  const denied = { allowed: false, reason: "placement_closed" as const };
  assert.deepEqual(resolvePermission("default", denied), denied);
  assert.equal(derivePlaceState(base({ permission: resolvePermission("default", denied) })).kind, "frozen");
});
