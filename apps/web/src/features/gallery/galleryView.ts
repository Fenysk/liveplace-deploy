...
export interface GalleryCardView {
  ...
  /**
   * Latest durable snapshot blob URL. When `thumbnailUrl` is null but this is
   * set, the card renders the snapshot client-side into a <canvas> element.
   */
  latestSnapshotUrl: string | null;
  /** Locale-formatted viewer count. */
  viewers: string;
  /** True when the streamer is currently live on Twitch (S1 FEN-1868). */
  twitchLive: boolean;
  width: number;
  height: number;
}
...
export function buildGalleryView(
  ...
  return {
    ...
    cards: result.page.map((item) => ({
      ...
      latestSnapshotUrl: item.latestSnapshotUrl,
      viewers: nf.format(item.viewerCount),
      twitchLive: (item as { twitchLive?: boolean }).twitchLive ?? false,
      width: item.width ?? 0,
