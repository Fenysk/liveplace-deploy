...
export function AuthButton({ onSignIn }: AuthButtonProps = {}): React.ReactElement {
  ...
  // A malformed session payload (e.g. the get-session endpoint answered with
  // HTML during a deploy/proxy hiccup) can be non-null yet carry no `user` —
  // destructuring it crashed the whole app into the error boundary (FEN-2154).
  // Only a session WITH a user counts as authenticated; anything else degrades
  // to loading/anonymous, never a render throw.
  ...
  //   authenticated → session confirmed (with a usable user)
  ...
  // authStatus === "authenticated" implies user != null (see authStatus derivation above).
