...
// FEN-665: when set, assert the served SPA bundle inlined this exact (absolute)
// Convex URL. A `/healthz` 200 only proves the static server is up — it says
// NOTHING about the JS. An empty VITE_CONVEX_URL ships `ConvexReactClient("")`,
// which throws at load and white-pages the app while every health check stays
// green. This check closes that blind spot: pass EXPECT_CONVEX_URL=https://<host>/convex.
const EXPECT_CONVEX_URL = process.env.EXPECT_CONVEX_URL ?? "";
...
/**
 * FEN-665: a green /healthz proves the static server is up but NOT that the SPA
 * actually mounts. Fetch the served index.html, follow its entry `<script
 * src=...>` bundle, and assert the expected absolute Convex URL is inlined. Vite
 * inlines VITE_CONVEX_URL as a verbatim string literal, so an empty/misconfigured
 * build (the white-page regression) simply won't contain it — and this fails.
 */
async function probeBundleConvexUrl(origin, expected) {
  const base = origin.replace(/\/$/, "");
  let html;
  try {
    const res = await Promise.race([fetch(`${base}/`), deadline(STEP_TIMEOUT_MS, "index.html")]);
    if (!res.ok) fail(`bundle check: GET / returned ${res.status} at ${base}/`);
    html = await res.text();
  } catch (err) {
    fail(`bundle check: GET / unreachable at ${base}/: ${err.message}`);
  }
  // Entry script: <script type="module" ... src="/assets/index-XXXX.js">
  const m = html.match(/<script[^>]+src="([^"]*\/assets\/index-[^"]+\.js)"/i);
  if (!m) fail(`bundle check: no entry /assets/index-*.js <script> in served HTML at ${base}/`);
  const entryUrl = m[1].startsWith("http") ? m[1] : `${base}${m[1].startsWith("/") ? "" : "/"}${m[1]}`;
  let js;
  try {
    const res = await Promise.race([fetch(entryUrl), deadline(STEP_TIMEOUT_MS, "entry bundle")]);
    if (!res.ok) fail(`bundle check: GET ${entryUrl} returned ${res.status}`);
    js = await res.text();
  } catch (err) {
    fail(`bundle check: entry bundle unreachable at ${entryUrl}: ${err.message}`);
  }
  if (!js.includes(JSON.stringify(expected)) && !js.includes(expected)) {
    fail(
      `bundle check: served entry (${entryUrl}) does NOT inline VITE_CONVEX_URL="${expected}". ` +
        `The build shipped an empty/wrong Convex URL → ConvexReactClient throws → white page (FEN-665).`,
    );
  }
  console.log(`  ✓ bundle inlines VITE_CONVEX_URL="${expected}" (${entryUrl.split("/").pop()})`);
}
...
async function run() {
  ...
  if (WEB_URL && EXPECT_CONVEX_URL) await probeBundleConvexUrl(WEB_URL, EXPECT_CONVEX_URL);
  else if (WEB_URL) console.log("  · bundle Convex-URL check skipped (set EXPECT_CONVEX_URL to enable)");
  ...
  if (welcome.protocolVersion !== 3) fail(`unexpected protocolVersion ${welcome.protocolVersion} (expected 3 — packages/protocol PROTOCOL_VERSION)`);
