...
/** Fail-closed resolution of the gateway auth flag (FEN-1018/AP-10).
 *  - unset            → "0" (Twitch OAuth — the safe default).
 *  - explicit "0"     → "0".
 *  - explicit "1"     → allowed ONLY with a noisy opt-in (`--allow-anon` flag or
 *                       LIVEPLACE_ANON_STACK=1); otherwise the deploy aborts so an
 *                       accidental "1" can never silently make the canvas globally
 *                       writable (regression FEN-978).
 *  - any other value  → rejected (no fail-open on typos like "true"/"yes"). */
function resolveAnonMode(e) {
  const raw = e.GATEWAY_AUTH_DISABLED;
  if (raw == null || raw === "" || raw === "0") return "0";
  if (raw !== "1") {
    throw new Error(
      `GATEWAY_AUTH_DISABLED="${raw}" is not a valid value — use "0" (Twitch OAuth, default) or "1" (anonymous smoke).`,
    );
  }
  const allowed = ARGS.has("--allow-anon") || e.LIVEPLACE_ANON_STACK === "1";
  if (!allowed) {
    throw new Error(
      'GATEWAY_AUTH_DISABLED=1 requested (ANONYMOUS stack — every socket accepted, canvas globally writable). ' +
        "This is a throwaway-smoke mode only and must be opted into explicitly: re-run with --allow-anon " +
        "(or set LIVEPLACE_ANON_STACK=1). Do NOT use it for the liveplace.tv prod deploy (FEN-978).",
    );
  }
  log("⚠️  ANONYMOUS auth stack enabled via explicit opt-in — every socket accepted, no Twitch. Throwaway smoke only.");
  return "1";
}

/** The full stack env pushed to Coolify. Mirrors .env.example; Twitch OAuth by
 *  default (anonymous mode is an explicit, noisy opt-in). Random secrets are
 *  generated when blank (and flagged so the operator
...
function buildStackEnv(c) {
  ...
  const stack = {
    ...
    // Auth — fail-closed default (FEN-1018/AP-10). When the var is unset the
    // tool pushes "0" (Twitch OAuth, gateway verifies the Convex JWT). The
    // anonymous stack (every socket accepted as "anon", canvas globally
    // writable) is opt-in ONLY and must be requested LOUDLY: set
    // GATEWAY_AUTH_DISABLED=1 *and* pass --allow-anon (or LIVEPLACE_ANON_STACK=1),
    // otherwise the deploy aborts. Historical regression FEN-978: a stray "1"
    // in prod made every pixel "Posé anonymement" — never re-push "1" to prod.
    GATEWAY_AUTH_DISABLED: resolveAnonMode(e),
    ...
    GATEWAY_PORT: "8080",
    // Gateway JWT verify (CA3, ADR-0002 / FEN-211). The gateway verifies authed
    // sockets' Convex JWTs offline against the better-auth JWKS. These MUST match
    // what better-auth emits, or every authed socket 401s (the gauge frame never
    // arrives and a signed-in canvas stays stuck on "La fresque arrive…"):
    //   - JWKS at `/api/auth/convex/jwks` on the Convex SITE origin (internal
    //     convex-backend:3211), NOT `${SITE}/.well-known/jwks.json` (Caddy serves
    //     the SPA HTML there -> jose finds no key for the `kid` -> 401).
    //   - `iss` = the public site URL (better-auth baseURL); empty -> skip check.
    //   - `aud` = `convex`. docker-compose.yml pins these on the gateway service
    //     too, so a missing/stale value here cannot silently re-break verify.
    CONVEX_JWKS_URL: e.CONVEX_JWKS_URL ?? "http://convex-backend:3211/api/auth/convex/jwks",
    GATEWAY_JWT_ISSUER: e.GATEWAY_JWT_ISSUER ?? base,
    GATEWAY_JWT_AUDIENCE: e.GATEWAY_JWT_AUDIENCE ?? "convex",
    CANVAS_WIDTH: e.CANVAS_WIDTH ?? "512",
    CANVAS_HEIGHT: e.CANVAS_HEIGHT ?? "512",
    // FEN-698: "default" is a reserved slug (canvases.ts RESERVED_SLUGS) — no
    // Convex canvas row can ever have it. Deploying "default" here causes the
    // worker to call applyFlush("default") → canvasFound:false → totalPixels=0
    // and lastActivityAt frozen forever. Set to the owner's Twitch login (e.g.
    // "fenysk") in deploy.env. Preflight warns if this is blank or "default".
...
async function triggerDeploy(api, uuid) {
  // NOTE (FEN-1101): `&force=true` (force_rebuild → docker build --no-cache +
  // recreate-all) was suspected to be the 503-rollover amplifier (FEN-946), but a
  // measured no-force redeploy of an UNCHANGED commit still 503'd "no available
  // server" for ~112 s — the whole build window. The downtime is therefore
  // force-INDEPENDENT (a structural Coolify dockercompose limitation: no rolling
  // deploy — the stack/edge is unavailable for the duration of the deploy). force
  // is kept ON because it preserves the hard-won anti-stale guarantee (a forced
  // rebuild can never serve a cached/stale bundle); dropping it does NOT buy
  // zero-downtime. True zero-downtime needs an architecture change — see
  // docs/runbooks/coolify-deploy.md §Zero-downtime.
...
async function main() {
  ...
  // FEN-698: "default" is a reserved slug — deploying it means the worker calls
  // applyFlush("default") → canvasFound:false → totalPixels=0, lastActivityAt frozen.
  if (!stack.GATEWAY_CANVAS_ID || stack.GATEWAY_CANVAS_ID === "default") {
    missing.push(
      "GATEWAY_CANVAS_ID must be set to the canvas owner's Twitch login (e.g. \"fenysk\"). " +
      "\"default\" is a reserved slug — the worker will call applyFlush(\"default\") → canvasFound:false → no pixels persist (FEN-698).",
    );
  }
  ...
  // FEN-665 GUARD: VITE_CONVEX_URL is a build-time arg inlined into the SPA. If it
  // is empty the bundle ships `new ConvexReactClient("")`, which throws at load and
  // white-pages prod while every HTTP health check stays green. NEVER push a build
  // that we already know is broken — abort loudly here (was only a warning before,
  // which is how FEN-617 reached prod).
  for (const k of ["VITE_CONVEX_URL", "VITE_CONVEX_SITE_URL"]) {
    const v = built.stack[k];
    if (!/^https?:\/\/.+/i.test(v ?? "")) {
      die(
        `${k} resolved to ${v ? `"${v}"` : "empty"} — refusing to deploy a white-page build. ` +
          `Set PUBLIC_BASE_URL (e.g. https://liveplace.tv) so VITE_* build args are real (FEN-665).`,
      );
    }
  }
