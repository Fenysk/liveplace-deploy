 ...
 *
 * Then ship via `node scripts/mirror-push.mjs` (guarded force-push to the mirror)
 * → Coolify force-deploy API → poll → verify the served surface (see
 * docs/runbooks/coolify-deploy.md). This is a ONE-TIME provisioning; ongoing
 * updates go through mirror-push, never through this script.
 *
...
async function main() {
  ...
  // Coolify builds only the dockercompose stack; it never runs GitHub Actions.
  // The deploy PAT is intentionally minimal (no `workflow` scope), so GitHub
  // rejects any push that creates/updates `.github/workflows/*`. Strip CI
  // workflow files from the publish snapshot — they are dead weight in the
  // deploy repo and would otherwise block every push (FEN-661 added ci.yml).
  rmSync(join(work, ".github", "workflows"), { recursive: true, force: true });

  // FEN-1629 guard: refuse to publish a bundle built from the neutral S0 UI
  // layer (design stripped — the FEN-1580 clobber). The elevation fingerprint
  // in tokens.css flips to `--elev-1: none` on S0; assertDesignFingerprint
  // throws on that BEFORE we commit/push. See docs/adr/0007.
  const tokensCss = join(work, "apps", "web", "src", "ui", "styles", "tokens.css");
  if (!existsSync(tokensCss)) die(`design fingerprint check: expected ${tokensCss} in bundle`);
  assertDesignFingerprint(readFileSync(tokensCss, "utf8"), { source: "deploy bundle tokens.css" });
  log("· design fingerprint OK (neobrutalism --elev present)");
  ...
  log("   next: node scripts/mirror-push.mjs   → guard → force-push bundle → Coolify force-deploy API → poll → verify served surface (docs/runbooks/coolify-deploy.md)");
}
