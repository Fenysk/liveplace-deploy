...
Gateway env (corrected FEN-211 — match what `@convex-dev/better-auth` actually
emits; the earlier `.well-known` / `CONVEX_SITE_URL`-issuer spec never verified a
real token and 401'd every authed socket):
`CONVEX_JWKS_URL = ${CONVEX_SITE_URL}/api/auth/convex/jwks` (better-auth serves
its JWKS there, on the Convex SITE origin — `.well-known/jwks.json` returns the
SPA HTML, so `jose` finds no key for the token's `kid` and the verify throws),
`GATEWAY_JWT_ISSUER = ${PUBLIC_SITE_URL}` (the `iss` claim is better-auth's
baseURL = public site URL, e.g. `https://liveplace.tv` — NOT an internal Convex
URL; leave empty to skip the `iss` check), `GATEWAY_JWT_AUDIENCE = convex`. In
the deployment the gateway reaches the JWKS internally at
`http://convex-backend:3211/api/auth/convex/jwks` (Caddy routes `/api/auth*` to
`convex-backend:3211`). Convex's own auth tolerated the old spec because it
resolves the issuer/JWKS via OIDC discovery; the gateway uses `jose`'s
exact-string `issuer` + a fixed `CONVEX_JWKS_URL`, so any drift = 401.
