...
cd /tmp/lp && rm -rf .github/workflows   # see workflow-scope note below
...
> (added by FEN-661), but Coolify never runs GitHub Actions — it only builds the
> dockercompose stack. A GitHub PAT without the `workflow` scope is *rejected* when a
> push would create/update any `.github/workflows/*` file (`refusing to allow a
> Personal Access Token to create or update workflow … without 'workflow' scope`).
> `coolify-wire-source.mjs` strips `.github/workflows/` from the publish snapshot
> automatically; the manual path must `rm -rf .github/workflows` before committing.
...
5. Triggers `GET /api/v1/deploy?uuid={uuid}&force=true` (instant deploy). See
   [Deploy downtime](#deploy-downtime--measured-root-cause-fen-1101) for why this
   briefly 503s and why `force` is kept on.
...
A real-time canvas should not flash a 503 on every rollout. Two facts govern this:

**1. A push to `liveplace-deploy@main` auto-triggers a Coolify rebuild.** Coolify
watches the deploy repo; *any* push to `main` (including a fresh `make-deploy-bundle`
snapshot) fires a webhook deploy on its own. So for a **code-only** change you do
**not** need to run `coolify-deploy.mjs` at all — pushing the snapshot is enough to
ship it. Run the script only when you must also change env vars (the push does not
`PATCH …/envs/bulk`). This is the "surprise" rollover in FEN-1098 — a snapshot push
*was* the deploy.

**2. Every Coolify deploy of this app 503s for ~the whole build window, and it is
`force`-INDEPENDENT.** This was measured on FEN-1101: a redeploy of an *unchanged*
commit with `force_rebuild=false` / `restart_only=false` (verified on the
deployment record) still returned `503 "no available server"` from the edge for
**~112 s** (`15:50:52 → 15:52:44Z`), clearing only after `build: finished`. The
earlier hypothesis (FEN-946/FEN-1098: `&force=true` recreating the edge proxy is
*the* amplifier) is therefore **refuted as the primary driver** — dropping `force`
does **not** buy zero-downtime. The window equals the deploy duration because
Coolify's `dockercompose` build pack has **no rolling / zero-downtime strategy**:
the stack (and the edge route to it) is unavailable while the deploy runs. `force`
is kept ON in the script because it preserves the anti-stale guarantee (a forced
rebuild can never serve a cached/stale bundle) at no extra downtime cost.

**Mitigations & options for true zero-downtime** (none are free — pick per FEN-1101
board decision; this is a D4 reverse-proxy / deploy-pipeline architecture call):

- **Registry-based deploy (recommended).** Build the web/gateway images in CI, push
  to a registry, and have Coolify *pull + recreate* instead of *build-on-host*. The
  ~2 min on-host build leaves the deploy critical path, collapsing the window to a
  few-second container swap. Cost: a CI build step + registry + compose `image:`
  refs. Biggest win, moderate effort.
- **Stable external edge.** Front the app with a reverse proxy that lives *outside*
  the Coolify-managed compose project (so it survives stack recreation) and retries
  / holds connections to the backend while it cycles. Cost: changes D4 topology.
- **Accept it.** The window self-heals in <2 min and is benign for a hobby-scale
  launch; just batch changes to deploy less often and avoid deploying at peak.

The `proxy` (Caddy) service carries a `healthcheck` (`/healthz` through Caddy to
web) so the orchestrator only reports the rollout done once the full public path
actually serves — defence-in-depth that helps NAS + the registry-deploy path, but
it does **not** by itself remove the build-window 503.

## Rollback

- **Bad deploy:** Coolify keeps the previous build. Roll back from the UI, or
  redeploy a known-good commit: `git push` that commit to `liveplace-deploy@main`
  (the push auto-triggers the deploy) or re-run the script. Rollback path is
  **unchanged** by FEN-1101 (no behaviour change to the deploy script).
