...
- **Symptôme :** un toggle de sécurité prend une valeur **permissive** quand la variable d'env n'est pas définie — ex. `GATEWAY_AUTH_DISABLED ?? "1"` (`scripts/coolify-deploy.mjs`) qui **désactive l'auth** à chaque déploiement où la variable manque.
