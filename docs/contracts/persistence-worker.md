...
- `snapshots` — periodic bin-palette-v1 blobs in Convex file storage (the durable
  canvas source of truth, ADR-0006). `by_canvas` → latest; `by_canvas_version`.
...
> **Retention/compaction** of the stream and the Convex snapshots is a separate
> contract: [retention.md](./retention.md) (stream MINID trim + MAXLEN backstop,
> snapshot keep-newest-5).
