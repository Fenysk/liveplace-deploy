...
```jsonc
{
  "canvasId": "k97…",        // Convex canvases._id — REQUIRED (FEN-1933): the gateway
                             // keys every Redis namespace + delta channel off this id
                             // (FEN-1563/FEN-1613); when absent it falls back to its
                             // default canvas (a dead namespace in multi-canvas prod).
  "slug": "streamerlogin",   // informational/log only since the _id migration
  "width": 100,
  "height": 100,
  "paletteSize": 16,
...
```jsonc
{ "canvasId": "k97…", "slug": "streamerlogin", "userId": "better-auth-id", "banned": false }
```
...
```jsonc
{ "canvasId": "k97…", "slug": "streamerlogin" }
```
...
```jsonc
{ "canvasId": "k97…", "slug": "streamerlogin", "frozen": true }
```
